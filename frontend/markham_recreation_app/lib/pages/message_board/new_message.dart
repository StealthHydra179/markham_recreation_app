...
class NewMessageBoard extends StatefulWidget {
  const NewMessageBoard({super.key});

  @override
  State<NewMessageBoard> createState() => _NewMessageBoardState();
}

class _NewMessageBoardState extends State<NewMessageBoard> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('New Message', style: TextStyle(color: globals.secondaryColor)),
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                Future<http.Response> response = globals.session.post(
                  Uri.parse('${globals.serverUrl}/api/new_message/${globals.campId}'), //+globals.camp_id.toString()
                ...
                response.then((http.Response response) {
                  if (response.statusCode == 200) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('New Message Saved'),
                        duration: Duration(seconds: 3),
                      ),
                    );
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Failed to Save New Message'),
                        duration: Duration(seconds: 3),
                      ),
                    );
                  }
