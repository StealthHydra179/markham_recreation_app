...
module.exports = function (expressServer, logger, postgresClient, dataSanitization, getPostgresConnected) {
    expressServer.get("/api/get_equipment_notes/:camp_id", authenticate, (req, res) => {
    ...
    expressServer.post("/api/new_equipment_note/:camp_id", authenticate, (req, res) => {
        let postgresConnected = getPostgresConnected();
        if (!postgresConnected) {
            res.status(500).send({ message: "Database not connected" });
            logger.error("Database not connected");
        ...
        logger.debug(
            `POST /api/new_equipment_note/:camp_id ${dataSanitization(req.params.camp_id)} ${dataSanitization(req.body.equip_note)} ${dataSanitization(req.body.equip_note_date)}`,
        );
        ...
        postgresClient.query(addQuery, addQueryValues, (err, res1) => {
            if (err) {
                logger.error("New equipment note error: ", err);
                res.status(500).send({ message: "New equipment note error" });
                return;
            }
    ...
    expressServer.post("/api/edit_equipment_note/:camp_id", authenticate, (req, res) => {
        let postgresConnected = getPostgresConnected();
        if (!postgresConnected) {
            res.status(500).send({ message: "Database not connected" });
            logger.error("Database not connected");
        ...
        const updateQuery =
            "UPDATE equipment_note SET equip_note = $1, equip_note_upd_date = $2, equip_note_upd_by = $3, equip_note_date = $5 WHERE equip_note_id = $4";
    ...
    expressServer.post("/api/delete_equipment_note/:camp_id", authenticate, (req, res) => {
        let postgresConnected = getPostgresConnected();
        if (!postgresConnected) {
            res.status(500).send({ message: "Database not connected" });
            logger.error("Database not connected");
