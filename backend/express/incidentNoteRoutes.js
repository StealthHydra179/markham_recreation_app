...
module.exports = function (expressServer, logger, postgresClient, dataSanitization, getPostgresConnected) {
    expressServer.get("/api/get_incident_notes/:camp_id", authenticate, (req, res) => {
    ...
    expressServer.post("/api/new_incident_note/:camp_id", authenticate, (req, res) => {
        let postgresConnected = getPostgresConnected();
        if (!postgresConnected) {
            res.status(500).send({ message: "Database not connected" });
            logger.error("Database not connected");
        ...
        postgresClient.query(addQuery, addQueryValues, (err, res1) => {
            if (err) {
                logger.error("New incident note error: ", err);
                res.status(500).send({ message: "New incident note error" });
                return;
            }
    ...
    expressServer.post("/api/edit_incident_note/:camp_id", authenticate, (req, res) => {
        let postgresConnected = getPostgresConnected();
        if (!postgresConnected) {
            res.status(500).send({ message: "Database not connected" });
            logger.error("Database not connected");
        ...
        const updateQuery =
            "UPDATE incident_note SET in_note = $1, in_note_upd_date = $2, in_note_upd_by = $3, in_note_date = $5 WHERE in_note_id = $4";
    ...
    expressServer.post("/api/delete_incident_note/:camp_id", authenticate, (req, res) => {
        let postgresConnected = getPostgresConnected();
        if (!postgresConnected) {
            res.status(500).send({ message: "Database not connected" });
            logger.error("Database not connected");
