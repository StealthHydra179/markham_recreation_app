module.exports = function (expressServer, logger, postgresClient, dataSanitization, getPostgresConnected) {
    expressServer.get("/api/get_supervisor_meeting_notes/:camp_id", (req, res) => {
    ...
    expressServer.post("/api/new_supervisor_meeting_note/:camp_id", (req, res) => {
        let postgresConnected = getPostgresConnected();
        if (!postgresConnected) {
            res.status(500).send({ message: "Database not connected" });
            logger.error("Database not connected");
        ...
        logger.debug(
            `POST /api/new_supervisor_meeting_note/:camp_id ${dataSanitization(req.params.camp_id)} ${dataSanitization(req.body.smeet_note)} ${dataSanitization(req.body.smeet_note_date)} ${dataSanitization(req.body.absence_date)} ${dataSanitization(req.body.followed_up)} ${dataSanitization(req.body.reason)}`,
        );
        ...
        postgresClient.query(addQuery, addQueryValues, (err, res) => {
            if (err) {
                logger.error("New supervisor meeting note error: ", err); // TODO send an error to the client // TODO figure out why logger.error gave undefined?
                return;
            }
    ...
    expressServer.post("/api/edit_supervisor_meeting_note/:camp_id", (req, res) => {
        let postgresConnected = getPostgresConnected();
        if (!postgresConnected) {
            res.status(500).send({ message: "Database not connected" });
            logger.error("Database not connected");
    ...
    expressServer.post("/api/delete_supervisor_meeting_note/:camp_id", (req, res) => {
        let postgresConnected = getPostgresConnected();
        if (!postgresConnected) {
            res.status(500).send({ message: "Database not connected" });
            logger.error("Database not connected");
